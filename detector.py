...
class yolox:
    ...
    def __init__(self, model_path):
        ...
        self.session = onnxruntime.InferenceSession(model_path,sess_opt,providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
    ...
    def detection(self, img):
        ...
        predictions = self.postprocess(output[0], (640,640))[0]
        ...
        dets = self.multiclass_nms(boxes_xyxy, scores, nms_thr=0.45, score_thr=0.8)
        if dets is not None:
            # 出力
            final_boxes, final_scores, final_cls_inds = dets[:, :4], dets[:, 4], dets[:, 5]
            # img = vis(img, final_boxes, final_scores, final_cls_inds,
            #                     conf=0.8 ,class_names=_CLASSES)
